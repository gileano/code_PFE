...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\linewidth]{figures/interaction_graph_weak_random_8q.png}
  \caption{Qubit-interaction graph for the same \texttt{weak\_random}
    ...
    partitions on. Isolated node placement reflects the layout
    algorithm's spacing, not physical distance.}
...
over the run's first two thirds, consistent with crowding-distance
selection trading convergence against diversity within the population
rather than strictly improving both every generation, before both
indicators plateau in the final third of the run. This is the kind of
...
\begin{figure}[htbp]
  ...
  \caption{Hypervolume and spread vs.\ generation for block $\{0,2,4\}$
